...
namespace Invector.vMelee
{
    ...
    public class vMeleeManager : vMonoBehaviour, IWeaponEquipmentListener
    {
        ...
        public virtual void OnDamageHit(ref vHitInfo hitInfo)
        {
            ...
            if (hitInfo.targetCollider.gameObject.TryGetComponent(out EmeraldAI.EmeraldAISystem target))
            {                 
                target.Damage((int)hitInfo.attackObject.damage.damageValue, EmeraldAI.EmeraldAISystem.TargetType.Player, transform, 400);
            }
        }
