...
Shader "GameTextures/GT_Specular"
{
	Properties
	{
		...
		_GlossinessMultiplier("Glossiness Multiplier", Range( 0 , 10)) = 1
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			o.Albedo = ( _BaseColorTint * tex2D( _Albedo, lerp_pom ) ).rgb;
			o.Emission = ( tex2D( _Emissive, lerp_pom ) * _EmissiveMultiplier ).rgb;
			float4 tex2D_specular = tex2D( _Specular, lerp_pom );
			o.Specular = tex2D_specular.rgb;
			#ifdef _INVERT_GLOSSINESS_ON
				float invert_glossiness_switch = ( 1.0 - tex2D_specular.a );
			#else
				float invert_glossiness_switch = tex2D_specular.a;
			#endif
			o.Smoothness = ( _GlossinessMultiplier * invert_glossiness_switch );
